...
void encode(Node *current,Node *arr, string str="") {
	...
	if(current->left != NULL){
		encode(current->left,arr,str+"0");
	}
	if(current->right != NULL){
		encode(current->right,arr,str+"1");
	}
	if(current->left == NULL && current->right == NULL){
		arr[current->data].enc=str;
		//cout << i << endl;
	}
}
...
int main() {
	while (cin) {
		...
		string truth;
		char lie;
		for(auto it = filename.rbegin(); it!=filename.rbegin()-4; it--){
			lie = *it;
			truth += lie;
		}
		...
		while(my_heap.size() > 1) {
            ...
            *second = my_heap.top();
            ...
            combine->count = first->count + second->count; //YOU: What should this intermediate node's count be?
            //cout << "New Node: " << combine->data << " - " << combine->count << endl;
            combine->left = first; //YOU: What should the left pointer point to?
            combine->right = second; //YOU: What should the right pointer point to?
		...
		for (int i = 0; i < 256; i++){
			...
			if (arr[i].count > 0) dict << arr[i].data << ": " << arr[i].enc << endl; //YOU: Write code to output "character: encoding", like "Q: 110111"
		}
		dict.close();
