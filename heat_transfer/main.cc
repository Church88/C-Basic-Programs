...
void clear_array(double *sheet,double value) {
	for (int i = 0; i < SIZE_X; i++)
		for (int j = 0; j < SIZE_Y; j++)
			sheet[index(i,j)] = value;
}

void print_array(double *sheet, bool clear=true, int precision = 2, int width=8) {
...
//		i+1 j+1 i-1 j-1 average i and j amongst those points
				sheet2[index(i,j)] = sheet[index(i,j)]; //The heart of your code will go HERE
			}
		}
